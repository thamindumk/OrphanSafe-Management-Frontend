...
import { useCreateProfileRequestMutation } from "../../slices/caseApiSlice";
...
const RequestChildProfile = () => {
  ...
  const [createProfileRequest, { isError, isLoading, isSuccess }] =
    ...
    if (requestDescription) {
      if (requestDescription.trim().length != 0) {
        try {
          ...
          console.log(request);
          const res = await createProfileRequest(request).unwrap();
